...
async def test_create_user_profile_with_fake_s3(
    ...
    user = User.create(email="test@email.com", raw_password="TestPassword123!", group_id=1)
    user.is_active = True
    db_session.add(user)
    await db_session.commit()

    stmt = select(User).where(User.email == "test@email.com")
...
async def test_create_user_profile_expired_token(client, jwt_manager):
    ...
    with patch("security.token_manager.datetime") as mock_datetime:
        ...
        expired_token = jwt_manager.create_access_token({"sub": str(1)})
...
async def test_admin_creates_user_profile(
    ...
    admin_token = jwt_manager.create_access_token({"sub": str(admin_user.id)})
    ...
    profile_url = f"/users/{regular_user.id}/profile/"
...
async def test_user_cannot_create_another_user_profile(
    ...
    user_1 = User.create(email="user1@mate.com", raw_password="User1Pass123!", group_id=1)
    user_1.is_active = True
    db_session.add(user_1)

    user_2 = User.create(email="user2@mate.com", raw_password="User2Pass123!", group_id=1)
    ...
    user_1_token = jwt_manager.create_access_token({"sub": str(user_1.id)})
    ...
    profile_url = f"/users/{user_2.id}/profile/"
...
async def test_profile_creation_invalid_name(client, jwt_manager, first_name, last_name, expected_error):
    access_token = jwt_manager.create_access_token({"sub": str(1)})
