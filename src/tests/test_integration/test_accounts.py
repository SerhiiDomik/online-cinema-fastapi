...
async def test_activate_account_success(client, db_session, seed_user_groups):
    ...
    registration_response = await client.post("/users/register/", json=registration_payload)
...
async def test_resend_activation_inactive_user(client, db_session, seed_user_groups):
    ...
    stmt_token1 = select(ActivationTokenModel).where(ActivationTokenModel.user_id == user.id)
    ...
    stmt_token2 = select(ActivationTokenModel).where(ActivationTokenModel.user_id == user.id)
...
async def test_reset_password_success(client, db_session, seed_user_groups):
    ...
    reset_request_response = await client.post("/users/password-reset/request/", json=reset_request_payload)
    ...
    reset_response = await client.post("/users/password-reset/complete/", json=reset_payload)
...
async def test_reset_password_invalid_token(client, db_session, seed_user_groups):
    ...
    reset_request_payload = {"email": registration_payload["email"]}
    response = await client.post("/users/password-reset/request/", json=reset_request_payload)
    ...
    response = await client.post("/users/password-reset/complete/", json=reset_complete_payload)
...
async def test_reset_password_expired_token(client, db_session, seed_user_groups):
    ...
    reset_response = await client.post("/users/password-reset/complete/", json=reset_complete_payload)
...
async def test_login_user_success(client, db_session, jwt_manager, seed_user_groups):
    ...
    assert access_token_data["sub"] == str(user.id), "Access token does not contain correct user ID."

    refresh_token_data = jwt_manager.decode_refresh_token(response_data["refresh_token"])
    assert refresh_token_data["sub"] == str(user.id), "Refresh token does not contain correct user ID."
...
async def test_logout_user_success(client, db_session, jwt_manager, seed_user_groups):
    ...
    stmt_refresh_before = select(RefreshTokenModel).where(RefreshTokenModel.token == refresh_token)
    ...
    assert refresh_record_before is not None, "Refresh token record should exist before logout"
    ...
    assert response_data["message"] == "Successfully logged out.", "Unexpected logout message"

    stmt_refresh_after = select(RefreshTokenModel).where(RefreshTokenModel.token == refresh_token)
    result_refresh_after = await db_session.execute(stmt_refresh_after)
    refresh_record_after = result_refresh_after.scalars().first()
    assert refresh_record_after is None, "Refresh token record should be deleted after logout"
...
async def test_logout_user_invalid_token(client, db_session, seed_user_groups):
    ...
    response_data = logout_response.json()
    assert response_data["detail"] == "Invalid refresh token.", "Expected 'Invalid refresh token.' detail"
